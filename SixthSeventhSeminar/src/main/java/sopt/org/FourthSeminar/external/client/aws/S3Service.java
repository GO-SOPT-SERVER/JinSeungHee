...
public class S3Service {
    ...
    public String uploadImage(MultipartFile multipartFile, String folder) {
        ...
        try (InputStream inputStream = multipartFile.getInputStream()) {
            amazonS3.putObject(new PutObjectRequest(bucket + "/" + folder + "/image", fileName, inputStream,
            ...
            throw new NotFoundException(Error.NOT_FOUND_SAVE_IMAGE_EXCEPTION,
                    Error.NOT_FOUND_SAVE_IMAGE_EXCEPTION.getMessage());
        }
    }

    public List<String> uploadImages(List<MultipartFile> multipartFileList, String folder) {
        // 스스로 실습해봅시다 ㅋㅋ
        return multipartFileList.stream().map(f -> uploadImage(f, folder)).collect(Collectors.toList());
    }
